...
const LOCAL_VEHICLE_DOCS_KEY = "safetydesk:vehicle-documents";
...
function valuesForLocal(form: Record<string, string>, userId: string, photoPath: string | null): Vehicle {
  return { user_id: userId, internal_number: form.internal_number || null, license_plate: form.license_plate, vehicle_type: form.vehicle_type || "Camión", brand: form.brand || null, model: form.model || null, vehicle_year: form.vehicle_year ? Number(form.vehicle_year) : null, color: form.color || null, mileage: form.mileage ? Number(form.mileage) : null, driver_name: form.driver_name || null, driver_document: form.driver_document || null, driver_license: form.driver_license || null, driver_license_expiry: form.driver_license_expiry || null, notes: form.notes || null, photo_path: photoPath, id: "", created_at: "", updated_at: "" };
}
...
function VehiculosPage() {
  ...
  const vehicles = useQuery({
    ...
    queryFn: async () => {
      if (localMode) return readLocal<Vehicle>(LOCAL_VEHICLES_KEY);
      const { data, error } = await supabase.from("vehicles").select("*").order("license_plate");
      if (error) { if (isMissingSupabaseTable(error)) { setLocalMode(true); return readLocal<Vehicle>(LOCAL_VEHICLES_KEY); } throw error; }
      return data;
    },
  });
  ...
  const remove = useMutation({
    mutationFn: async (vehicle: Vehicle) => {
      if (localMode) { writeLocal(LOCAL_VEHICLES_KEY, readLocal<Vehicle>(LOCAL_VEHICLES_KEY).filter((item) => item.id !== vehicle.id)); return; }
...
function VehicleModal({ vehicle, userId, localMode, onClose }: { vehicle: Vehicle | null; userId: string; localMode: boolean; onClose: () => void }) {
  const qc = useQueryClient();
  ...
  const docs = useQuery({
    ...
    queryFn: async () => { if (localMode) return readLocal<VehicleDocument>(LOCAL_VEHICLE_DOCS_KEY).filter((document) => document.vehicle_id === savedVehicle!.id); const { data, error } = await supabase.from("vehicle_documents").select("*").eq("vehicle_id", savedVehicle!.id).order("created_at", { ascending: false }); if (error) { if (isMissingSupabaseTable(error)) return readLocal<VehicleDocument>(LOCAL_VEHICLE_DOCS_KEY).filter((document) => document.vehicle_id === savedVehicle!.id); throw error; } return data; },
  });
  ...
  async function saveVehicle(event: React.FormEvent) {
    ...
    try {
      if (localMode) {
        const photoPath = photo ? await fileToDataUrl(photo) : savedVehicle?.photo_path ?? null;
        const localVehicle: Vehicle = { ...valuesForLocal(form, userId, photoPath), id: savedVehicle?.id ?? crypto.randomUUID(), created_at: savedVehicle?.created_at ?? new Date().toISOString(), updated_at: new Date().toISOString() };
        const current = readLocal<Vehicle>(LOCAL_VEHICLES_KEY);
        writeLocal(LOCAL_VEHICLES_KEY, savedVehicle ? current.map((item) => item.id === savedVehicle.id ? localVehicle : item) : [...current, localVehicle]);
        setSavedVehicle(localVehicle); qc.invalidateQueries({ queryKey: ["vehicles"] }); toast.success(savedVehicle ? "Vehículo actualizado" : "Vehículo guardado en este navegador"); return;
      }
    ...
    } catch (error) { if (isMissingSupabaseTable(error)) { setLocalMode(true); toast.info("Supabase aún no está configurado. Guardá nuevamente para usar el modo local."); } else toast.error(error instanceof Error ? error.message : "No se pudo guardar el vehículo"); } finally { setSaving(false); }
  }
  ...
  async function uploadDocument(event: React.FormEvent) {
    ...
    try { if (localMode) { const localDocument: VehicleDocument = { id: crypto.randomUUID(), vehicle_id: savedVehicle.id, user_id: userId, document_type: docType, file_name: docFile.name, file_path: await fileToDataUrl(docFile), expiry_date: docExpiry || null, created_at: new Date().toISOString() }; writeLocal(LOCAL_VEHICLE_DOCS_KEY, [...readLocal<VehicleDocument>(LOCAL_VEHICLE_DOCS_KEY), localDocument]); toast.success("Documentación cargada en este navegador"); setDocFile(null); setDocExpiry(""); qc.invalidateQueries({ queryKey: ["vehicle_documents", savedVehicle.id] }); return; } const upload = await supabase.storage.from("vehicle-files").upload(path, docFile); if (upload.error) throw upload.error; const { error } = await supabase.from("vehicle_documents").insert({ vehicle_id: savedVehicle.id, user_id: userId, document_type: docType, file_name: docFile.name, file_path: path, expiry_date: docExpiry || null }); if (error) throw error; toast.success("Documentación cargada"); setDocFile(null); setDocExpiry(""); qc.invalidateQueries({ queryKey: ["vehicle_documents", savedVehicle.id] }); } catch (error) { toast.error(error instanceof Error ? error.message : "No se pudo cargar la documentación"); }
  }

  async function downloadDocument(document: VehicleDocument) { if (localMode) { const link = window.document.createElement("a"); link.href = document.file_path; link.download = document.file_name; link.click(); return; } const { data, error } = await supabase.storage.from("vehicle-files").createSignedUrl(document.file_path, 300); if (error) toast.error(error.message); else window.open(data.signedUrl, "_blank", "noopener,noreferrer"); }
